...
void message_sig (int signo){
    if(signo == SIGUSR1){
        volatile QEvt *qe = Q_NEW(QEvt, MESSAGE_SIG);
        QACTIVE_POST((QActive*)AO_Blinky, qe,(QActive*)AO_Blinky);
    }
    signal(SIGUSR1, message_sig);
}
...
int main() {
    ...
    static QF_MPOOL_EL(QEvt) tmpQ[sizeof(QEvt)*10];
    QF_init(); /* initialize the framework */
    QF_poolInit(tmpQ, sizeof(tmpQ), sizeof(tmpQ[0]));
    signal(SIGUSR1, message_sig);
